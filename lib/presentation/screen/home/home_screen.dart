...
import 'package:mt/presentation/screen/calendar/calendar_screen.dart';
import 'package:mt/presentation/screen/home/home_state.dart';
import 'package:mt/presentation/screen/privacy.dart';
import 'package:mt/presentation/screen/card_detail/card_detail_screen.dart';
import 'package:mt/presentation/screen/card_list/card_list_screen.dart';
...
const List<_MenuPage> _menuPages = [
  _MenuPage(
      title: 'Calendar',
      description: 'Quickly preview and prepare your schedule',
      icon: Icons.event),
  _MenuPage(
      title: 'All Cards',
      description: 'Display all active Cards',
      icon: Icons.list),
  _MenuPage(
      title: 'Settings',
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    final initSettings = InitializationSettings(
      AndroidInitializationSettings('mt_logo'),
  ...
  Future _onSelectNotification(String payload) async {
    ...
    if (payload != null) {
      final cards = await dependencies.cardInteractor.active.first;
      final notificationCard = cards.firstWhere(
          (e) => e.addedDate.toIso8601String().compareTo(payload) == 0);

      await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) =>
              CardDetailScreen(card: notificationCard, editable: true),
        ),
      );
    }
  }
            ...
            content: Column(
              ...
              children: ColorfulTheme.values
                  ...
                  .map((color) => InkWell(
                        ...
                        highlightColor: ColorfulApp.of(context)
                            .themeDataFromEnum(color)
                            .pale
                            .withOpacity(0.4),
                        splashColor: ColorfulApp.of(context)
                            .themeDataFromEnum(color)
                            .medium
                            .withOpacity(0.4),
                        onTap: () =>
                            ColorfulApp.of(context).updateColorTheme(color),
                        child: _buildColorDialogTile(
                          text: capitalize(enumToStringSingle(color)),
                          mainColor: ColorfulApp.of(context)
                              .themeDataFromEnum(color)
                              .pale,
                          borderColor: ColorfulApp.of(context)
                              .themeDataFromEnum(color)
                              .bleak,
                        ),
                      ))
      ...
      children: <Widget>[
        ...
        Text(
          ...
          style: TextStyle().copyWith(
              color: ColorfulApp.of(context).colors.bleak, fontSize: 22.0),
        ),
      ],
    );
  }
  ...
  Widget _buildDescriptionContent(HomeState state) {
    return Padding(
      ...
      child: Text(
        ...
        textAlign: TextAlign.center,
        style: TextStyle().copyWith(
            color: ColorfulApp.of(context).colors.bleak, fontSize: 16.0),
      ),
    );
  }
  ...
  Widget _buildColorDialogTile(
      {String text, Color mainColor, Color borderColor}) {
    return Padding(
